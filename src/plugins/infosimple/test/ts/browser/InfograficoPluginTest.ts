...
UnitTest.asynctest('browser.tinymce.plugins.infosimple.InfosimplePluginTest', function () {
  ...
  const teardown = function (editor) {
    delete editor.settings.infosimples_dimensions;
    delete editor.settings.file_browser_callback;
    delete editor.settings.infosimples_list;
    delete editor.settings.infosimples_class_list;
    delete editor.settings.document_base_url;
    delete editor.settings.infosimple_advtab;
    delete editor.settings.infosimples_caption;
  };
  ...
  suite.test('Default image dialog on empty editor', function (editor) {
    editor.setContent('');
    editor.execCommand('mceInfosimple', true);
  ...
  suite.test('Infosimple dialog infosimple_dimensions: false', function (editor) {
    editor.settings.infosimples_dimensions = false;
    editor.setContent('');
    editor.execCommand('mceInfosimple', true);
  ...
  if (Env.ceFalse) {
    suite.test('All image dialog ui options on empty editor', function (editor) {
      editor.settings.infosimples_caption = true;
      editor.settings.infosimples_list = [
        { title: 'link1', value: 'link1' },
        { title: 'link2', value: 'link2' }
      ];

      editor.settings.infosimples_class_list = [
  ...
  suite.test('Infosimple recognizes relative src url and prepends relative infosimple_prepend_url setting.', function (editor) {
    let win, elementId, element;

    editor.settings.infosimples_prepend_url = 'testing/images/';
    editor.setContent('');
    editor.execCommand('mceInfosimple', true);
    ...
    LegacyUnit.equal(
      cleanHtml(editor.getContent()),
      '<p><img src="' + editor.settings.infosimples_prepend_url + 'src" alt="alt" /></p>'
    );
  });

  suite.test('Infosimple recognizes relative src url and prepends absolute infosimple_prepend_url setting.', function (editor) {
    let win, elementId, element;

    editor.settings.infosimples_prepend_url = 'http://abc.local/images/';
    editor.setContent('');
    editor.execCommand('mceInfosimple', true);
  ...
  suite.test('Advanced image dialog border option on editor with content', function (editor) {
    editor.settings.infosimple_advtab = true;
    editor.settings.infosimples_dimensions = false;
