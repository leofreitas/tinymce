...
import InfosimplePlugin from 'tinymce/plugins/infosimple/Plugin';
import ModernTheme from 'tinymce/themes/modern/Theme';
import { document } from '@ephox/dom-globals';
import { Element } from '@ephox/sugar';

UnitTest.asynctest('browser.tinymce.plugins.infosimple.InfosimpleListTest', function () {
  ...
  TinyLoader.setup(function (editor, onSuccess, onFailure) {
    ...
    Pipeline.async({}, [
      Logger.t('click image list, check that source changes, change source and check that image list changes', GeneralSteps.sequence([
        tinyApis.sSetSetting('infosimple_list', [
        ...
        Chain.asStep({}, [
          ...
          UiFinder.cFindIn('label:contains("Infosimple list") + div > button'),
          Mouse.cClick
        ]),
        ...
        Chain.asStep({}, [
          Chain.fromParent(tinyUi.cWaitForPopup('wait for dialog', 'div[role="dialog"]'),
            [
              ...
              Chain.fromChains([
                UiFinder.cFindIn('label:contains("Infosimple list") + div > button > span:contains("Cat")')
              ])
            ]
          )
        ])
