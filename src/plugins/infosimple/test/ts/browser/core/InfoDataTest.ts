...
import { read, write, create, isInfosimple, defaultData, getStyleValue } from 'tinymce/plugins/infosimple/core/InfosimpleData';
import { Merger, Obj, Arr } from '@ephox/katamari';
import { DOMUtils } from 'tinymce/core/api/dom/DOMUtils';

UnitTest.asynctest('browser.tinymce.plugins.image.core.InfosimpleDataTest', (success, failure) => {
  const cSetHtml = (html) => {
  ...
  const cReadFromInfosimple = Chain.mapper(function (elm: Element) {
  ...
  const cWriteToInfosimple = Chain.op(function (data: any) {
  ...
  const cAssertInfosimple = Chain.op(function (data: any) {
    RawAssertions.assertEq('Should be an image', true, isInfosimple(data.image.dom()));
  });
