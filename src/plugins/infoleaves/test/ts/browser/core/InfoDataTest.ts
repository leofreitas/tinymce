...
import { read, write, create, isInfoLeaves, defaultData, getStyleValue } from 'tinymce/plugins/infoleaves/core/InfoLeavesData';
import { Merger, Obj, Arr } from '@ephox/katamari';
import { DOMUtils } from 'tinymce/core/api/dom/DOMUtils';

UnitTest.asynctest('browser.tinymce.plugins.image.core.InfoLeavesDataTest', (success, failure) => {
  const cSetHtml = (html) => {
  ...
  const cReadFromInfoLeaves = Chain.mapper(function (elm: Element) {
  ...
  const cWriteToInfoLeaves = Chain.op(function (data: any) {
  ...
  const cAssertInfoLeaves = Chain.op(function (data: any) {
    RawAssertions.assertEq('Should be an image', true, isInfoLeaves(data.image.dom()));
  });
