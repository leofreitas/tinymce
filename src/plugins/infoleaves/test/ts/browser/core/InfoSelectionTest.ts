...
import { insertOrUpdateInfoLeaves } from 'tinymce/plugins/infoleaves/core/InfoLeavesSelection';
import { Element } from '@ephox/sugar';

UnitTest.asynctest('browser.tinymce.plugins.image.core.InfoLeavesSelectionTest',  (success, failure) => {
  Theme();
  Plugin();

  const sUpdateInfoLeavesOrFigure = (editor, data) => {
    return Step.sync(() => {
      insertOrUpdateInfoLeaves(editor, Merger.merge({
        src: 'image.png',
        alt: '',
        title: '',
        width: '200',
  ...
  TinyLoader.setup(function (editor, onSuccess, onFailure) {
    ...
    Pipeline.async({}, [
      Logger.t('Insert image, size 100x100', GeneralSteps.sequence([
        ...
        sUpdateInfoLeavesOrFigure(editor, {
          src: 'image.png',
          height: '100',
          width: '100'
        }),
