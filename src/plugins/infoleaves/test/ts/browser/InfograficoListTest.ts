...
UnitTest.asynctest('browser.tinymce.plugins.infografico.InfoLeavesListTest', function () {
    ...
    Pipeline.async({}, [
      Logger.t('click image list, check that source changes, change source and check that image list changes', GeneralSteps.sequence([
        ...
        Chain.asStep({}, [
          ...
          UiFinder.cFindIn('label:contains("InfoLeaves list") + div > button'),
          Mouse.cClick
        ]),
        ...
        Chain.asStep({}, [
          Chain.fromParent(tinyUi.cWaitForPopup('wait for dialog', 'div[role="dialog"]'),
            [
              ...
              Chain.fromChains([
                UiFinder.cFindIn('label:contains("InfoLeaves list") + div > button > span:contains("Cat")')
              ])
            ]
          )
        ])
